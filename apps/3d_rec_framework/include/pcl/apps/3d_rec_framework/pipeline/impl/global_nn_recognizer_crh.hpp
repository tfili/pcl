...
template<template<class > class Distance, typename PointInT, typename FeatureT>
  void
  pcl::rec_3d_framework::GlobalNNCRHRecognizer<Distance, PointInT, FeatureT>::getPose (ModelT & model, int view_id, Eigen::Matrix4d & pose_matrix)
  {
...

      {
        ...
        if (do_CRH_)
        {
          ...
          for (int i = 0; i < num_n; ++i)
          {
            ...
            std::vector < Eigen::Matrix4d, Eigen::aligned_allocator<Eigen::Matrix4d> > roll_transforms;
            crha.getTransforms (roll_transforms);

            //create object hypothesis
            for (size_t k = 0; k < roll_transforms.size (); k++)
            {
              Eigen::Matrix4d final_roll_trans (roll_transforms[k] * model_view_pose);
              models_->push_back (m);
